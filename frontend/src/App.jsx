...
import {
  ...
  Database, Zap, Filter, Info, Heart, GitCompare
} from 'lucide-react';
...
function AIReport({trainResult}){
  ...
  const REPORT_TYPES=[
    {id:'scientific', label:'Scientific Report',
     desc:'800–1000 word detailed thesis-style report with model-by-model analysis, feature importance, literature citations, and ICH Q8/Q9 regulatory context.'},
    {id:'executive',  label:'Executive Report',
     desc:'400–500 word non-technical briefing for VPs and C-suite. Business impact, plain language, deployment recommendation, no jargon.'},
    {id:'summary',    label:'Quick Summary',
     desc:'200–250 word punchy results snapshot — headline numbers, top features, one recommendation. Ideal for team meetings or slide notes.'},
  ];
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <GlowCard accent={C.purple} className="slide-up">
        <div style={{fontSize:17,fontWeight:700,color:C.text,marginBottom:4}}>AI Report Generator</div>
        <div style={{fontSize:13,color:C.textMid,marginBottom:20,lineHeight:1.6}}>
          Choose a report type below. All reports use your actual trained results — nothing hardcoded.
          API key is loaded from your <code style={{...S.mono,fontSize:12,padding:'1px 6px',background:C.bg4,borderRadius:4,color:C.blue}}>.env</code> file.
        </div>

        {/* Report type selector */}
        <div style={{...S.label,marginBottom:10}}>Report Type</div>
...
export default function App(){
  ...
  // step 4 = BMS1 vs BMS2 static page (always accessible)
  const canGo=s=>{
    if(s===0||s===4)return true;
    ...
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',flexDirection:'column'}}>
      ...
      <div style={{background:C.bg2,borderBottom:`1px solid ${C.border}`,
        ...
        <div style={{background:'#fff',borderRadius:8,padding:'5px 10px',display:'inline-flex',alignItems:'center',flexShrink:0}}>
          <img src="/bms-logo.png" alt="BMS" style={{height:28,objectFit:'contain'}}
            onError={e=>{e.target.style.display='none';}}/>
        </div>

        {/* Title */}
        <div style={{flexShrink:0}}>
          <div style={{fontSize:16,fontWeight:700,color:C.text,letterSpacing:'-0.2px'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,marginTop:2}}>
            ...
            <span style={{fontSize:11,color:C.textDim}}>MS Data Science · Rutgers × BMS</span>
          </div>
        </div>

        {/* Built with love */}
        <div style={{display:'flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:20,
          background:'rgba(244,114,182,0.1)',border:'1px solid rgba(244,114,182,0.25)',flexShrink:0}}>
          <span style={{fontSize:11,color:'#f472b6',fontWeight:500}}>Built with</span>
          <Heart size={13} color='#f472b6' fill='#f472b6'/>
          <span style={{fontSize:11,color:'#f472b6',fontWeight:500}}>by Team Data Minds</span>
        </div>
          ...
          {UI_STEPS.map((s,i)=>{
            ...
            return(
              <React.Fragment key={s.id}>
                {i===4&&<div style={{width:1,height:24,background:C.border,margin:'0 6px'}}/>}
                <button onClick={()=>accessible&&setStep(s.id)} disabled={!accessible}
                  style={{display:'flex',alignItems:'center',gap:7,padding:'7px 14px',borderRadius:9,
                    border:`1px solid ${active?(isBMS?C.purple:C.blue):done?C.green:isBMS?C.border:C.border}`,
                    background:active?(isBMS?C.purpleL:C.blueL):done?C.greenL:'transparent',
                    cursor:accessible?'pointer':'not-allowed',transition:'all 0.2s',
                    fontSize:12,fontWeight:active?600:400,
                    color:active?(isBMS?C.purple:C.blue):done?C.green:isBMS?C.purple+80:C.textDim,
                    fontFamily:'DM Sans,sans-serif',opacity:accessible?1:0.4}}>
                  {done?<CheckCircle size={14} color={C.green}/>:<Icon size={14}/>}
                  {s.label}
                </button>
                {i<UI_STEPS.length-1&&i!==3&&<ChevronRight size={14} color={C.textDim} style={{margin:'0 2px'}}/>}
              </React.Fragment>
            );
          })}
        </div>
