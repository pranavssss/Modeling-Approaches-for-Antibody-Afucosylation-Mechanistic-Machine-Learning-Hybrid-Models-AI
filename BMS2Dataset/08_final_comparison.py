...
# GPR sample cap — GPR is O(n³) so we limit training size
MAX_GPR_SAMPLES = 300
...
print("--- GPR ---")
t0 = time.time()
gpr_subsample_idx = np.random.RandomState(42).choice(
    len(X_train_s), min(MAX_GPR_SAMPLES, len(X_train_s)), replace=False
)
gpr = GaussianProcessRegressor(
    kernel=ConstantKernel(1.0) * RBF(length_scale=1.0) + WhiteKernel(noise_level=1.0),
    n_restarts_optimizer=3, random_state=42, alpha=0.1
)
gpr.fit(X_train_s[gpr_subsample_idx], y_train[gpr_subsample_idx])
t_gpr = time.time() - t0
...
for frac in train_sizes_frac:
    n = max(int(frac * len(X_train_s)), 10)
    X_sub = X_train_s[:n]
    y_sub = y_train[:n]
    ...
    for name, m_dict in models.items():
        model_obj = m_dict["model"]
        try:
            # FIX: For GPR, always cap the subset at MAX_GPR_SAMPLES
            # to prevent O(n³) explosion on large datasets
            if name == "GPR":
                n_gpr = min(n, MAX_GPR_SAMPLES)
                gpr_idx = np.random.RandomState(42).choice(n, n_gpr, replace=False) if n > n_gpr else np.arange(n)
                m_clone = clone(model_obj)
                m_clone.fit(X_sub[gpr_idx], y_sub[gpr_idx])
            else:
                m_clone = clone(model_obj)
            ...
            print(f"  Warning: {name} failed at {frac:.0%} data: {e}")
            r2 = np.nan
...
for name in models:
    ...
    eff_score = normalize_score(efficiency_at_20pct[name], list(efficiency_at_20pct.values()))
    rob_score = normalize_score(robustness_scores[name], list(robustness_scores.values()))
